...
2. **One machine hosts.** One person runs the `server`. It needs a reachable
   address (a public IP, or a home router with one UDP port forwarded). That one
   command does two jobs: it's the meeting point everyone checks in with, and it
   joins the LAN itself as an ordinary member (it takes the first address,
   `10.66.0.1`). Everyone else points their `join` at the host's address.

3. **Introductions.** When a machine checks in (with a shared secret), the host
   hands it a virtual address and tells everyone who else is on the network,
   including the public endpoint it saw each one arrive from. That's all the host
   does for peer-to-peer links - the actual traffic between two other peers never
   goes through it.

4. **UDP hole punching.** Two machines behind home routers normally can't reach
   each other directly. Using the endpoints the host reported, both sides fire UDP
...
5. **Forwarding.** Once the path is open, packets read off the TUN device are
...
On the host (needs a reachable address and root; it hosts *and* joins as
`10.66.0.1`):
...
On each other machine (needs root/Administrator to create the TUN):

```sh
sudo hamachi-like join --server vpn.example.com:7777 --secret s3cret
```

Then just use the virtual addresses: `ping 10.66.0.1`, `ssh user@10.66.0.3`, etc.
